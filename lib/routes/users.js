...
module.exports = function(db){    
    ...
    router.get('', function(req, res, next){
        ...
        collection.find({},{ password: false, admin:false }).toArray(function(err, result) {
            if (err) next(err);
            result.forEach(element => {
                element.baseTopic = "/"+baseTopic(element._id.toString());
            });
            res.json(result);
        });
    });
     ...
     * {
     *    username: string
     *    password: string (plain text, it will be encrypted automatically at creation ),
     *    admin: boolean
     * } 
     */
